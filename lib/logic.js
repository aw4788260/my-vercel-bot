...
const firestore = new Firestore({
  ...
  credentials: {
    ...
    // Note: Assuming you are still using the @@ format for the key
...
/**
 * NEW: Parses a single line of CSV-formatted question text.
 * Format: question,option1,option2,option3,correct_answer_number
 */
function parseSingleQuestion(line) {
    try {
        const parts = line.split(',').map(p => p.trim());
        if (parts.length < 4) return null; // Must have question, at least 2 options, and 1 answer number
...
async function promptForQuestions(chatId, examId, messageId = null) {
  // NEW: Updated instructions for the new CSV format
...
export async function handleCategoryRename(chatId, newName, userState) {
    ...
    await firestore.runTransaction(async (transaction) => {
        ...
        transaction.set(newDocRef, { ...oldDoc.data(), name: newNameTrimmed });
...
export async function addBulkQuestionsFromText(chatId, text, userState) {
    // NEW: Updated to use the new CSV format
...
export async function deleteQuestion(chatId, examId, docId, messageId) {
    // FIXED: This function is now transactional and re-orders subsequent questions.
    try {
        await firestore.runTransaction(async (transaction) => {
        ...
        await sendMessage(chatId, "🚨 حدث خطأ أثناء حذف السؤال.");
    }
}
...
// =============================================================================
//                             STUDENT QUIZ LOGIC (NEW & UNIFIED)
// =============================================================================
...
export async function formatUserScores(scores) {
...
export async function startQuiz(callbackQuery) {
    ...
    const prepared = questionsRaw.map((q, idx) => ({ ...q, questionText: escapeMarkdown(q.questionText || "") }));
    
    // NEW: Unified quiz flow
    const quizData = {
        examId,
        userName,
...
export async function sendQuestion(userId, userState) {
    ...
    const message = await sendMessage(userId, `*السؤال ${currentQuestionIndex + 1} من ${questions.length}:*\n\n${question.questionText}`, null, "Markdown");
    
    const sentPoll = await sendPoll(userId, `Question ${currentQuestionIndex + 1}`, question.options, 'quiz', question.correctOptionIndex, timePerQuestion);
...
export async function handlePollAnswer(pollAnswer) {
    ...
    if (!userState || userState.state !== 'taking_exam' || pollAnswer.poll_id !== userState.data.lastPollId) {
        return; // Old poll or not in an exam
    }

    // Immediately stop the poll so user can't change their answer
...
export async function finishQuiz(userData) {
    const { userId, userName, examId, score, questions } = userData;
