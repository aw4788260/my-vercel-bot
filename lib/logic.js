...
export async function sendQuestion(userId, userState) {
    ...
    const sentPoll = await sendPoll(userId, `السؤال ${currentQuestionIndex + 1} من ${questions.length}: ${question.questionText}`, question.options, 'quiz', question.correctOptionIndex, timePerQuestion);
    
    if (sentPoll && sentPoll.poll) {
        await setUserState(userId, 'taking_exam', { 
            ...userState.data,
            lastPollId: sentPoll.poll.id,
            lastMessageId: sentPoll.message_id, // Polls are messages themselves
        });
    }
}
export async function handlePollAnswer(pollAnswer) {
    ...
    if (userState.data.currentQuestionIndex >= userState.data.questions.length) {
        await finishQuiz(userId.toString(), userState.data);
    } else {
