...
/**
 * Parses a single line of CSV-formatted question text.
...
export async function isAdmin(userId) {
    if (userId.toString() === PRIMARY_ADMIN_ID) {
        return true;
    }
...
export async function sendStudentMenu(chatId, messageId = null) {
    ...
    // NEW: Added back to main menu button for all users
    const keyboard = { 
        inline_keyboard: [
...
export async function handleCategoryRename(chatId, newName, userState) {
    ...
    // NEW: Query for all exams in the old category
    ...
    // Add updates for each exam to the batch
...
export async function handleNewExamName(chatId, text) {
    ...
    // NEW: Set default settings and skip prompts
    const examData = {
        examId,
        allowRetake: true,
        timePerQuestion: 0
...
export function formatUserScores(scores) {
    ...
    for (const examId in groupedScores) {
        ...
        groupedScores[examId].sort((a, b) => b.timestamp - a.timestamp).forEach(attempt => {
            ...
            } else if (attempt.timestamp && attempt.timestamp.seconds) { // Firestore Timestamp object
...
export async function sendQuestion(userId, userState) {
    ...
    // NEW: No question number in the title
    ...
    if (sentPoll && sentPoll.poll) {
        await setUserState(userId, 'taking_exam', { 
            ...
            lastQuestionTimestamp: Date.now(), // For timeout check
        });
    }
}
...
export async function checkTimedOutQuizzes() {
  ...
  for (const doc of statesSnapshot.docs) {
    ...
    if (data && data.lastQuestionTimestamp && data.timePerQuestion > 0) {
      ...
      if (timeElapsed > data.timePerQuestion + 2) { // 2 seconds buffer
        console.log(`User ${userId} timed out on a question.`);
        try {
            const userState = await getUserState(userId);
            // Check again to make sure state hasn't changed
