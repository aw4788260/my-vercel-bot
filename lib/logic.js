...
async function promptForQuestions(chatId, examId, messageId = null) {
  // NEW: Updated instructions to include forwarding
  const text = `👍 تم حفظ إعدادات الاختبار: **${examId}**.\n\nالآن قم بإضافة الأسئلة بأحد الطريقتين:\n\n1️⃣ **الكتابة اليدوية (CSV):**\nأرسل كل سؤال في سطر بصيغة:\n` +
  "`السؤال,الخيار1,الخيار2,رقم الإجابة`\n\n" +
  "2️⃣ **التحويل المباشر:**\nقم **بتحويل (Forward)** استطلاعات من نوع اختبار (Quiz Polls) من أي قناة إلى هنا. يمكنك تحويل حتى 100 سؤال دفعة واحدة.\n\n" +
...
function buildCategoryTreeList(allCategories, parentId = null, prefix = '') {
    ...
    for (const category of children) {
        result.push({ ...category, indentedName: `${prefix}${category.name}` });
        result = result.concat(buildCategoryTreeList(allCategories, category.name, `${prefix}    `));
    }
    return result;
}
...
// =============================================================================
// All functions in this block are the same as the previous final version
...
export async function clearUserState(userId) { await firestore.collection('userStates').doc(userId.toString()).delete(); }
...
export async function sendCategoryManagementMenu(chatId, messageId) { const text = "🗂️ إدارة الفئات\n\nاختر الإجراء المطلوب:"; const keyboard = { inline_keyboard: [ [{ text: "➕ إضافة فئة رئيسية", callback_data: "category_add_toplevel" }], [{ text: "➕ إضافة فئة فرعية", callback_data: "category_add_subcategory" }], [{ text: "✏️ تعديل / عرض الفئات", callback_data: "category_edit_list" }], [{ text: "🔄 تغيير ترتيب الفئات الرئيسية", callback_data: "category_reorder" }], [{ text: "🔙 رجوع للوحة التحكم", callback_data: "admin_panel" }]] }; await editMessageText(chatId, messageId, text, keyboard); }
export async function promptForNewCategoryName(chatId, messageId, isSubcategory = false, parentId = null) { const state = isSubcategory ? 'awaiting_subcategory_name' : 'awaiting_toplevel_category_name'; await setUserState(chatId, state, { parentId }); await editMessageText(chatId, messageId, "📝 يرجى إرسال اسم الفئة الجديدة."); }
export async function promptForParentCategory(chatId, messageId) { const allCategories = await getAllCategories(); if (allCategories.length === 0) { return await editMessageText(chatId, messageId, "لا توجد فئات رئيسية لإضافة فئة فرعية تحتها. قم بإنشاء فئة رئيسية أولاً.", { inline_keyboard: [[{ text: "🔙 رجوع", callback_data: "manage_categories" }]] }); } const categoryTree = buildCategoryTreeList(allCategories); const text = "اختر الفئة الأب التي تريد إضافة الفئة الجديدة تحتها:"; const keyboard = { inline_keyboard: categoryTree.map(cat => ([{ text: cat.indentedName, callback_data: `category_select_parent:${cat.name}` }])) }; keyboard.inline_keyboard.push([{ text: "🔙 رجوع", callback_data: "manage_categories" }]); await editMessageText(chatId, messageId, text, keyboard); }
export async function handleNewCategory(chatId, text, userState) { const categoryName = text.trim(); if (!categoryName) return; const { parentId } = userState.data; const categories = await getAllCategories(); if (categories.some(c => c.name === categoryName)) { return await sendMessage(chatId, "⚠️ هذه الفئة موجودة بالفعل. يرجى اختيار اسم آخر."); } const siblingCategories = categories.filter(c => (c.parentId || null) === (parentId || null)); const newOrder = siblingCategories.length > 0 ? Math.max(...siblingCategories.map(c => c.displayOrder || 0)) + 1 : 1; const categoryData = { name: categoryName, displayOrder: newOrder, parentId: parentId || null }; await firestore.collection('categories').doc(categoryName).set(categoryData); await clearUserState(chatId); await sendMessage(chatId, `✅ تم إضافة الفئة "${categoryName}" بنجاح.`); await sendAdminMenu(chatId); }
export async function listCategoriesForEditing(chatId, messageId, categoryId = null) { const allCategories = await getAllCategories(); const childCategories = allCategories.filter(cat => (cat.parentId || null) === categoryId).sort((a, b) => (a.displayOrder || 99) - (b.displayOrder || 99)); if (allCategories.length === 0 && !categoryId) { return await editMessageText(chatId, messageId, "لا توجد فئات لعرضها.", { inline_keyboard: [[{ text: "🔙 رجوع", callback_data: "manage_categories" }]] }); } const text = categoryId ? `أنت في فئة: *${categoryId}*` : "اختر فئة لعرض محتواها أو تعديلها:"; const keyboardRows = []; for (const cat of childCategories) { keyboardRows.push([ { text: `📁 عرض محتوى "${cat.name}"`, callback_data: `admin_browse_categories:${cat.name}` }, { text: `✏️ تعديل`, callback_data: `category_edit:${cat.name}` } ]); } let backCallback = 'manage_categories'; if (categoryId) { const currentCategory = allCategories.find(c => c.name === categoryId); backCallback = currentCategory?.parentId ? `admin_browse_categories:${currentCategory.parentId}` : 'category_edit_list'; } keyboardRows.push([{ text: "🔙 رجوع", callback_data: backCallback }]); const keyboard = { inline_keyboard: keyboardRows }; await editMessageText(chatId, messageId, text, keyboard, "Markdown"); }
export async function sendSingleCategoryEditMenu(chatId, categoryName, messageId) { const text = `ماذا تريد أن تفعل بالفئة: *${categoryName}*؟`; let parentId = (await firestore.collection('categories').doc(categoryName).get()).data()?.parentId; const backCallback = parentId ? `admin_browse_categories:${parentId}` : 'category_edit_list'; const keyboard = { inline_keyboard: [ [{ text: "📝 إعادة تسمية", callback_data: `category_rename:${categoryName}` }], [{ text: "🗑️ حذف", callback_data: `category_delete_confirm:${categoryName}` }], [{ text: "🔙 رجوع", callback_data: backCallback }] ]}; await editMessageText(chatId, messageId, text, keyboard, "Markdown"); }
export async function promptForCategoryRename(chatId, oldName, messageId) { await setUserState(chatId, 'awaiting_category_rename', { oldName }); await editMessageText(chatId, messageId, `أرسل الاسم الجديد للفئة "${oldName}":`); }
export async function handleCategoryRename(chatId, newName, userState) { const { oldName } = userState.data; const newNameTrimmed = newName.trim(); if (!newNameTrimmed || oldName === newNameTrimmed) { await clearUserState(chatId); return await sendMessage(chatId, "تم إلغاء العملية."); } const batch = firestore.batch(); const oldDocRef = firestore.collection('categories').doc(oldName); const newDocRef = firestore.collection('categories').doc(newNameTrimmed); const examsToUpdateQuery = firestore.collection('exams').where('categoryName', '==', oldName); const childCategoriesToUpdateQuery = firestore.collection('categories').where('parentId', '==', oldName); const [examsSnapshot, childCategoriesSnapshot, oldDoc] = await Promise.all([ examsToUpdateQuery.get(), childCategoriesToUpdateQuery.get(), oldDocRef.get() ]); examsSnapshot.forEach(doc => { batch.update(doc.ref, { categoryName: newNameTrimmed }); }); childCategoriesSnapshot.forEach(doc => { batch.update(doc.ref, { parentId: newNameTrimmed }); }); if (oldDoc.exists) { batch.set(newDocRef, { ...oldDoc.data(), name: newNameTrimmed }); batch.delete(oldDocRef); } await batch.commit(); await clearUserState(chatId); await sendMessage(chatId, `✅ تم تغيير اسم الفئة وتحديث ${examsSnapshot.size} اختبار و ${childCategoriesSnapshot.size} فئة فرعية.`); await sendAdminMenu(chatId); }
export async function confirmCategoryDelete(chatId, categoryName, messageId) { const text = `🚨 *تحذير!* هل أنت متأكد من حذف الفئة "*${categoryName}*؟ سيتم حذف كل الفئات الفرعية والاختبارات بداخلها.`; const keyboard = { inline_keyboard: [[{ text: "🗑️ نعم، احذف الفئة", callback_data: `category_delete_execute:${categoryName}` }], [{ text: "❌ تراجع", callback_data: `category_edit:${categoryName}` }]] }; await editMessageText(chatId, messageId, text, keyboard, "Markdown"); }
export async function handleCategoryDelete(chatId, categoryName, messageId) { await firestore.collection('categories').doc(categoryName).delete(); await editMessageText(chatId, messageId, `✅ تم حذف الفئة "*${categoryName}*" بنجاح.`, { inline_keyboard: [[{ text: "🔙 رجوع", callback_data: "manage_categories" }]] }, "Markdown"); }
export async function promptForCategoryReorder(chatId, messageId) { const allCategories = await getAllCategories(); const topLevelCategories = allCategories.filter(c => !c.parentId); const categoryNames = topLevelCategories.map(c => c.name).join("\n"); const text = "🔄 *تغيير ترتيب الفئات الرئيسية*\n\nالترتيب الحالي هو:\n" + categoryNames + "\n\nللتغيير، أرسل قائمة بالأسماء مفصولة بفاصلة (,) بالترتيب الجديد الذي تريده."; await setUserState(chatId, 'awaiting_category_order'); await editMessageText(chatId, messageId, text, { inline_keyboard: [[{ text: "🔙 رجوع", callback_data: "manage_categories" }]] }, "Markdown"); }
export async function handleCategoryReorder(chatId, text) { const newOrderNames = text.split(',').map(name => name.trim()); const batch = firestore.batch(); newOrderNames.forEach((name, index) => { if (name) { const docRef = firestore.collection('categories').doc(name); batch.update(docRef, { displayOrder: index + 1 }); } }); await batch.commit(); await clearUserState(chatId); await sendMessage(chatId, "✅ تم تحديث ترتيب الفئات الرئيسية بنجاح."); await sendAdminMenu(chatId); }

// =============================================================================
//                             EXAM & QUESTION MANAGEMENT
// =============================================================================
export async function adminBrowseCategoriesForSelection(chatId, messageId, parentId = null, mode = 'new', examId = '') {
    const allCategories = await getAllCategories();
    const childCategories = allCategories
        .filter(cat => (cat.parentId || null) === parentId)
        .sort((a, b) => (a.displayOrder || 99) - (b.displayOrder || 99));

    const text = parentId ? `اختر فئة فرعية أو اختر الفئة الحالية "${parentId}"`: "اختر فئة رئيسية للاختبار:";
    const keyboardRows = [];
    const callbackPrefix = mode === 'new' ? `category_select_new` : `category_select_existing:${examId}`;

    if (parentId) {
        keyboardRows.push([{ text: `✅ اختيار "${parentId}"`, callback_data: `${callbackPrefix}:${parentId}` }]);
    }

    childCategories.forEach(cat => {
        keyboardRows.push([
            { text: `✅ اختيار "${cat.name}"`, callback_data: `${callbackPrefix}:${cat.name}` },
            { text: `📁 عرض الفئات الفرعية`, callback_data: `admin_browse_cat_for_select:${cat.name}:${mode}:${examId}` }
        ]);
    });

    let backCallback = (mode === 'new') ? 'admin_add_exam' : `admin_edit_exam:${examId}`;
    if (parentId) {
        const currentCategory = allCategories.find(c => c.name === parentId);
        backCallback = currentCategory?.parentId
            ? `admin_browse_cat_for_select:${currentCategory.parentId}:${mode}:${examId}`
            : `admin_select_exam_category:${mode}:${examId}`; 
    }
    ...
    if (messageId) {
        ...
        await sendMessage(chatId, text, { inline_keyboard: keyboardRows });
    }
}
...
export async function startAddExamFlow(chatId, messageId) {
    await setUserState(chatId, 'awaiting_exam_name');
    const text = "📝 لنبدأ بإضافة اختبار جديد.\n\nالرجاء إرسال **اسم الاختبار** (سيكون فريدًا لكل اختبار).";
    if (messageId) await editMessageText(chatId, messageId, text, null, "Markdown");
    else await sendMessage(chatId, text, null, "Markdown");
}
export async function handleNewExamName(chatId, text) {
    ...
    await adminBrowseCategoriesForSelection(chatId, null, null, 'new', examId);
}
...
/**
 * NEW: Generic handler for adding questions from text or forwarded polls.
 */
export async function handleQuestionInput(chatId, message, userState, isAddingAfter = false) {
    let successCount = 0;
    let failCount = 0;
    let parsedQuestions = [];

    // Case 1: Forwarded Quiz Poll
    if (message.poll && message.poll.type === 'quiz') {
        const poll = message.poll;
        parsedQuestions.push({
            questionText: poll.question,
            options: poll.options.map(o => o.text),
            correctOptionIndex: poll.correct_option_id,
        });
        successCount = 1;
    }
    // Case 2: CSV Text
        ...
        for (const line of lines) {
            ...
            const parsedQuestion = parseSingleQuestionCSV(line);
            if (parsedQuestion) {
    ...
    if (successCount === 0 && failCount === 0) return; // No valid input found

    // Now, add the parsed questions to the correct place
    if (isAddingAfter) {
        ...
        await firestore.runTransaction(async (transaction) => {
            ...
            subsequentDocs.forEach(doc => {
                transaction.update(doc.ref, { order: doc.data().order + parsedQuestions.length });
            });
            
            parsedQuestions.forEach((q, index) => {
                const newQuestionRef = firestore.collection('questions').doc();
                const newQuestionData = { ...q, examId, order: afterOrder + 1 + index };
                transaction.set(newQuestionRef, newQuestionData);
            });
            
            transaction.update(examRef, { questionCount: FieldValue.increment(successCount) });
        });
        await sendMessage(chatId, `✅ تم إضافة ${successCount} سؤال بنجاح بعد السؤال رقم ${afterOrder}.`);

    } else { // Adding to the end (new exam or bulk add)
        const { examId } = userState.data;
        if (examId) { // Adding to existing exam
            await firestore.runTransaction(async (transaction) => {
                const examRef = firestore.collection('exams').doc(examId);
                ...
                parsedQuestions.forEach(q => {
                    currentOrder++;
                    const newQuestionRef = firestore.collection('questions').doc();
                    transaction.set(newQuestionRef, { ...q, examId, order: currentOrder });
                });
                transaction.update(examRef, { questionCount: currentOrder });
            });
        } else { // Adding to a new exam (stored in state)
            if (!userState.data.questions) userState.data.questions = [];
            userState.data.questions.push(...parsedQuestions);
...
export async function adminBrowseCategoriesForExams(chatId, messageId, categoryId = null) {
    const allCategories = await getAllCategories();
    const subCategories = allCategories
        .filter(cat => (cat.parentId || null) === categoryId)
        .sort((a, b) => (a.displayOrder || 99) - (b.displayOrder || 99));
    
    const examsSnapshot = await firestore.collection('exams').where('categoryName', '==', categoryId).get();
    const exams = examsSnapshot.docs.map(doc => doc.data());

    const text = categoryId ? `أنت في فئة: *${categoryId}*` : "تصفح الفئات لعرض الاختبارات:";
    const keyboardRows = [];

    subCategories.forEach(cat => {
        keyboardRows.push([{ text: `📁 ${cat.name}`, callback_data: `admin_browse_exams:${cat.name}` }]);
    });
    exams.forEach(exam => {
        keyboardRows.push([{ text: `📝 ${exam.examId}`, callback_data: `admin_exam_details:${exam.examId}` }]);
    });
    
    let backCallback = 'admin_panel'; 
    if (categoryId) {
        const currentCategory = allCategories.find(c => c.name === categoryId);
        backCallback = currentCategory?.parentId
            ? `admin_browse_exams:${currentCategory.parentId}`
            : 'admin_browse_exams';
    }
    keyboardRows.push([{ text: "🔙 رجوع", callback_data: backCallback }]);
    
    await editMessageText(chatId, messageId, text, { inline_keyboard: keyboardRows }, "Markdown");
}
export async function showExamDetails(chatId, examId, messageId) {
    ...
    const keyboard = {
        inline_keyboard: [
            ...
            [{ text: "⬅️ رجوع", callback_data: `admin_browse_exams:${exam.categoryName}` }]
        ]
    };
...
export async function promptToChangeExamCategory(chatId, examId, messageId) {
    await adminBrowseCategoriesForSelection(chatId, messageId, null, 'existing', examId);
}
...
export async function handleReplaceQuestion(chatId, text, userState) {
    ...
    const parsedQuestion = parseSingleQuestionCSV(text);
...
export async function promptForNextQuestion(chatId, examId) {
    ...
    const text = `*إضافة أسئلة جديدة لاختبار ${examId}*\n\nأرسل الآن الأسئلة الجديدة (كل سؤال في سطر) بصيغة CSV أو قم بتحويل استطلاعات جاهزة.`;
...
export async function promptForQuestionAfter(chatId, examId, afterOrderStr) {
    ...
    const text = `*إضافة سؤال بعد السؤال رقم ${afterOrder}*\n\nأرسل الآن السؤال الجديد بصيغة CSV أو قم بتحويل استطلاع جاهز.`;
    await sendMessage(chatId, text, null, "Markdown");
}
export async function deleteQuestion(chatId, examId, docId, messageId) {
    try {
        await firestore.runTransaction(async (transaction) => {
            ...
            const subsequentQuestionsQuery = firestore.collection('questions').where('examId', '==', examId).where('order', '>', deletedOrder);
...
export async function deleteExam(chatId, examId, messageId) {
    ...
    await batch.commit();
    await editMessageText(chatId, messageId, `✅ تم حذف الاختبار "*${examId}*" وجميع أسئلته بنجاح.`, { inline_keyboard: [[{ text: "🔙 رجوع لقائمة الاختبارات", callback_data: "admin_browse_exams" }]] }, "Markdown");
}
...
export async function getScoresForUser(userId) {
    ...
    return snapshot.docs.map(doc => doc.data()).sort((a,b) => b.timestamp - a.timestamp);
}
...
export async function studentBrowseCategories(chatId, messageId, categoryId = null) {
    const allCategories = await getAllCategories();
    const subCategories = allCategories
        .filter(cat => (cat.parentId || null) === categoryId)
        .sort((a, b) => (a.displayOrder || 99) - (b.displayOrder || 99));
    ...
    if (categoryId) {
        const examsSnapshot = await firestore.collection('exams').where('categoryName', '==', categoryId).get();
        exams = examsSnapshot.docs.map(doc => doc.data());
    }

    const keyboardRows = [];
    subCategories.forEach(cat => {
        keyboardRows.push([{ text: `📁 ${cat.name}`, callback_data: `student_browse_categories:${cat.name}` }]);
    });
    exams.forEach(exam => {
        keyboardRows.push([{ text: `📝 ${exam.examId}`, callback_data: `show_exam_confirm:${exam.examId}` }]);
    });

    let text;
    if (subCategories.length === 0 && exams.length === 0) {
        if (categoryId) {
            text = `لا توجد فئات فرعية أو اختبارات في فئة *${categoryId}*.`;
        } else {
            text = "لا توجد أي فئات اختبارات متاحة حاليًا.";
        }
    } else {
        text = categoryId ? `أنت في فئة: *${categoryId}*` : "🗂️ يرجى اختيار فئة الاختبارات:";
    }

    let parentCategory = null;
    if (categoryId) {
        parentCategory = allCategories.find(cat => cat.name === categoryId);
    }
    
    const backCallback = parentCategory?.parentId 
        ? `student_browse_categories:${parentCategory.parentId}` 
        : 'student_panel';
    keyboardRows.push([{ text: "🔙 رجوع", callback_data: backCallback }]);
...
export async function showExamConfirmation(chatId, examId, messageId) {
    ...
    const keyboard = { inline_keyboard: [[{ text: "🚀 بدء الاختبار الآن", callback_data: `confirm_start_exam:${examId}` }], [{ text: "⬅️ رجوع", callback_data: `student_browse_categories:${exam.categoryName}` }]]};
    await editMessageText(chatId, messageId, text, keyboard, "Markdown");
}
