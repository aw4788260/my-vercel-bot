...
export async function listCategoriesForEditing(chatId, messageId, categoryId = null) {
    const allCategories = await getAllCategories();
    // Filter for children of the current categoryId (or top-level if null)
    const childCategories = allCategories.filter(cat => (cat.parentId || null) === categoryId);
    ...
    // Create a row with two buttons for each child category
    for (const cat of childCategories) {
        keyboardRows.push([
            { text: `📁 ${cat.name}`, callback_data: `admin_browse_categories:${cat.name}` },
    ...
    if (categoryId) {
        ...
        backCallback = currentCategory?.parentId 
            ...
            : 'category_edit_list'; // Go back to the top-level list
    }
...
export async function promptForCategoryReorder(chatId, messageId) {
    const allCategories = await getAllCategories();
    // Reorder only top-level categories for now
...
export async function studentBrowseCategories(chatId, messageId, categoryId = null) {
    const allCategories = await getAllCategories();
    // FIXED: Filter for top-level categories, including old ones with no parentId field
    const subCategories = allCategories
        .filter(cat => (cat.parentId || null) === categoryId)
        .sort((a, b) => (a.displayOrder || 99) - (b.displayOrder || 99));
    
    let exams = [];
    // FIXED: Fetch exams for the current category, even if it's a top-level one
    if (categoryId) {
    ...
    let text;
    // FIXED: Better messaging for empty states
