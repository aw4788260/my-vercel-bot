...
export async function handleMessage(message) {
    ...
    // Handle forwarded polls or text for question input states
    if (userState && isUserAdmin && (userState.state === 'awaiting_questions' || userState.state === 'awaiting_next_question' || userState.state === 'awaiting_question_after')) {
        const isAddingAfter = userState.state === 'awaiting_question_after';
        await logic.handleQuestionInput(chatId, message, userState, isAddingAfter);
        return;
    }
    
    if (!message.text) return; // Ignore non-text messages after the poll check
    ...
    // State-based handlers for admins (that are not question inputs)
