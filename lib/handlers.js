...
export async function handleMessage(message) {
    ...
    if (text.startsWith("/addadmin") && chatId === PRIMARY_ADMIN_ID) {
        ...
        if (parts.length < 2 || !/^\d+$/.test(parts[1])) {
            return await logic.sendMessage(chatId, "⚠️ صيغة خاطئة. استخدم: `/addadmin USER_ID`");
        }
        await logic.addAdmin(parts[1]);
        return await logic.sendMessage(chatId, `✅ تم إضافة ${parts[1]} كمشرف جديد.`);
    }
    if (text.startsWith("/deladmin") && chatId === PRIMARY_ADMIN_ID) {
        const parts = text.split(' ');
        if (parts.length < 2 || !/^\d+$/.test(parts[1])) {
            return await logic.sendMessage(chatId, "⚠️ صيغة خاطئة. استخدم: `/deladmin USER_ID`");
        }
        await logic.deleteAdmin(parts[1]);
        return await logic.sendMessage(chatId, `✅ تم حذف المشرف ${parts[1]}.`);
    }
    ...
    if (isUserAdmin) {
        if (text === "/usercount") {
            ...
            return await logic.sendMessage(chatId, `📊 العدد الكلي للمستخدمين هو: *${count}*`, null, "Markdown");
        }
        if (text.startsWith("/getresults")) {
            const parts = text.split(' ');
            if (parts.length < 2 || !parts[1]) {
                return await logic.sendMessage(chatId, "⚠️ صيغة خاطئة. يرجى استخدام:\n`/getresults USER_ID`", null, "Markdown");
            }
            ...
            return await logic.sendMessage(chatId, replyText, null, "Markdown");
        }
    }
...
export async function handleCallbackQuery(callbackQuery) {
    ...
    if (isUserAdmin) {
        switch (action) {
            ...
            case 'category_add': await logic.promptForNewCategoryName(chatId, messageId); return;
            case 'category_edit_list': await logic.listCategoriesForEditing(chatId, messageId); return;
            ...
            case 'admin_add_exam': await logic.startAddExamFlow(chatId); return;
            case 'finish_adding_questions': await logic.finishAddingQuestions(chatId); return;
            case 'admin_list_exams': await logic.listExamsForAdmin(chatId, messageId, parseInt(param1 || '1')); return;
    ...
    switch (action) {
        ...
        case 'noop': return;
        default: console.log(`Unknown or unauthorized callback action: ${action}`);
    }
}
