...
export async function handleMessage(message) {
    ...
    // NEW: Centralized question input handler for all question-adding states
    if (userState && isUserAdmin && (
        userState.state === 'awaiting_questions' || 
        userState.state === 'awaiting_next_question' || 
        userState.state === 'awaiting_question_after' ||
        userState.state === 'awaiting_replacement_question'
        )) {
        // This function now handles both text and forwarded polls
        await logic.handleQuestionInput(chatId, message, userState);
        return;
    }
    
    if (!message.text) return; // Ignore non-text messages after the poll/question check
    ...
    if (text === "/start") {
        ...
        await logic.clearUserState(chatId); // Clear state on /start just in case
...
export async function handleCallbackQuery(callbackQuery) {
    ...
    // Cancel operation can be done by anyone in a state
    ...
    if (isUserAdmin) {
        switch (action) {
            ...
            // NEW: This button now finalizes the process for all question-adding types
